...
#/* <DTS2016012907052 guochenguang 20160129 begin */
...
for board_id in ${BOARD_ID_TI_24152[@]}
do
    cat /proc/app_info | grep "$board_id"
    if [ $? -eq 0 ];then
        log_type=$log_type_ti_24152
        break
    fi
done
#/* DTS2016012907052 guochenguang 20160129 end> */
...
#Ti log begin
    ...
    do
        ...
        echo -n `cat /sys/class/power_supply/bms/cal_capacity` >> $log_path
        echo -n , >> $log_path
...
#/* <DTS2016012907052 guochenguang 20160129 begin */
#-----------------------------------------------------------------------------------------------------------------------------------
elif [ $log_type -eq $log_type_ti_24152 ];then
#-----------------------------------------------------------------------------------------------------------------------------------
#Ti_24152 log begin
    echo -n "Date, Time, Voltage Current SOC RM FCC II SI Temp FLAG Status Qmax, Reg[0] Reg[1] Reg[2] Reg[3] Reg[4], usbonline, usbscope, usbtype, usbcurrentmax, batterypresent, hot_current_limit, health, status, soc_user, charging_enabled, brightness, <cpu0_1_2_3>, cpu0_cpuinfo_cur_freq, cpu0_scaling_max_freq, zone2_temp, zone4_temp, therm_cpu, Mode" >> $log_path
    echo >> $log_path
    chmod 755 /data/log/chargelog/chargelog.csv
    while true
    do
